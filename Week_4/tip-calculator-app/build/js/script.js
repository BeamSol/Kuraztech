document.addEventListener('DOMContentLoaded', () => {
  ...
  const zeroWarning = document.getElementById('warning');
  ...
  numOfPeopleInput.addEventListener('input', () => {
    numOfPeople = parseFloat(numOfPeopleInput.value) || 0;
    calculateTip();
  });
  ...
  resetButton.addEventListener('click', (event) => {
    ...
    tipValue = 0.0;
    numOfPeople = 1;
    // zeroWarning.classList.add('hidden'); // Hide the warning message
    updateDisplays(0.0, 0.0);
  });

  function calculateTip() {
    if (numOfPeople === 0) {
      zeroWarning.classList.remove('hidden'); // Show the warning message
      updateDisplays(0.0, 0.0);
    } else {
      zeroWarning.classList.add('hidden'); // Hide the warning message
